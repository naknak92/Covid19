...
if selected_option == options[5]:
    ...
    fig_regression_all = go.Figure()
    fig_regression_all.add_trace(go.Scatter(x=X['total_cases'], y=y, mode='markers', name='Données réelles'))
    fig_regression_all.add_trace(go.Scatter(x=X['total_cases'], y=predicted_vaccinations, mode='lines', name='Régression linéaire'))

    fig_regression_all.update_layout(title='Régression linéaire du nombre de vaccinations en France par rapport aux cas totaux de COVID-19',
                                     xaxis_title='Nombre total de cas de COVID-19',
                                     yaxis_title='Nombre total de personnes vaccinées',
                                     title_font=dict(size=16),
                                     legend=dict(x=0.7, y=0.9))
    ...
    st.plotly_chart(fig_regression_all)
